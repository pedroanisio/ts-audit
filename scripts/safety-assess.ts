...
export function generateAssessmentReport(constraints: EngineeringConstraints): string {
	...
	lines.push(`| Branch Coverage | ${constraints.testing.branchCoverage}% |`);
	lines.push(`| MC/DC Coverage | ${constraints.testing.mcdcCoverage}% |`);
	lines.push(`| Unit Testing | ${constraints.testing.unitTestingRequired ? "Required" : "Optional"} |`);
	lines.push(`| Integration Testing | ${constraints.testing.integrationTestingRequired ? "Required" : "Optional"} |`);
	lines.push(`| Security Testing | ${constraints.testing.securityTestingRequired ? "Required" : "Optional"} |`);
	lines.push(`| Fault Injection | ${constraints.testing.faultInjectionRequired ? "Required" : "Optional"} |`);
	lines.push(`| Independent Verification | ${constraints.testing.independentVerificationRequired ? "Required" : "Optional"} |`);
	lines.push(``);
	...
	lines.push(`| Requirement | Value |`);
	lines.push(`|-------------|-------|`);
	lines.push(`| Redundancy | ${constraints.architecture.redundancyRequired ? constraints.architecture.redundancyType : "Not required"} |`);
	...
	lines.push(`| Audit Trail | ${constraints.architecture.auditTrailRequired ? constraints.architecture.auditTrailLevel : "Not required"} |`);
	lines.push(``);
	...
	lines.push(`| Requirement | Value |`);
	lines.push(`|-------------|-------|`);
	lines.push(`| Code Review | ${constraints.process.codeReviewRequired ? "Required" : "Optional"} |`);
	...
	lines.push(`| Coding Standard | ${constraints.process.codingStandardRequired ? constraints.process.codingStandard : "Not required"} |`);
...
function runDemo() {
	...
	console.log(
		"                    SAFETY ASSESSMENT FRAMEWORK DEMO                           ",
	);
	console.log(
		"═══════════════════════════════════════════════════════════════════════════════\n",
	);

	// Demo 1: Internal tool
	console.log("▶ EXAMPLE 1: Internal Admin Tool\n");
	const internalResult = assessProjectConstraints(QUICK_PROFILES.internalTool());
	console.log(`  Level: ${internalResult.integrityLevel} - ${internalResult.levelDescription}`);
	console.log(`  Test Coverage Required: ${internalResult.testing.statementCoverage}%`);
	console.log(`  Reviewers Required: ${internalResult.process.minReviewers}`);
	console.log(`  Redundancy: ${internalResult.architecture.redundancyRequired ? "Yes" : "No"}\n`);

	// Demo 2: B2B SaaS
	console.log("▶ EXAMPLE 2: B2B SaaS Platform\n");
	const saasResult = assessProjectConstraints(QUICK_PROFILES.b2bSaas());
	console.log(`  Level: ${saasResult.integrityLevel} - ${saasResult.levelDescription}`);
	console.log(`  Test Coverage Required: ${saasResult.testing.statementCoverage}%`);
	console.log(`  Reviewers Required: ${saasResult.process.minReviewers}`);
	console.log(
		`  Redundancy: ${saasResult.architecture.redundancyRequired ? saasResult.architecture.redundancyType : "No"}\n`,
	);

	// Demo 3: Payment System
	console.log("▶ EXAMPLE 3: Payment Processing System\n");
	const paymentResult = assessProjectConstraints(QUICK_PROFILES.paymentSystem());
	console.log(`  Level: ${paymentResult.integrityLevel} - ${paymentResult.levelDescription}`);
	console.log(`  Test Coverage Required: ${paymentResult.testing.statementCoverage}%`);
	console.log(`  Reviewers Required: ${paymentResult.process.minReviewers}`);
	console.log(
		`  Redundancy: ${paymentResult.architecture.redundancyRequired ? paymentResult.architecture.redundancyType : "No"}`,
	);
	console.log(`  Rollback Time: ${paymentResult.scoping.rollbackTimeRequirement}\n`);

	// Demo 4: Medical Device
	console.log("▶ EXAMPLE 4: Medical Device Software\n");
	const medicalResult = assessProjectConstraints(QUICK_PROFILES.medicalDevice());
	console.log(`  Level: ${medicalResult.integrityLevel} - ${medicalResult.levelDescription}`);
	console.log(`  Test Coverage Required: ${medicalResult.testing.statementCoverage}%`);
	console.log(`  MC/DC Coverage: ${medicalResult.testing.mcdcCoverage}%`);
	console.log(`  Formal Methods: ${medicalResult.process.staticAnalysisLevel}`);
	console.log(
		`  Independent V&V: ${medicalResult.testing.independentVerificationRequired ? "Yes" : "No"}\n`,
	);

	// Full report for SaaS
	console.log(
		"\n───────────────────────────────────────────────────────────────────────────────",
	);
	console.log(
		"                         FULL REPORT: B2B SaaS                                ",
	);
	console.log(
		"───────────────────────────────────────────────────────────────────────────────\n",
	);
	console.log(generateAssessmentReport(saasResult));
	...
	console.log(generateChecklist(saasResult));
	...
	const comparison = compareIntegrityLevels(IntegrityLevel.LEVEL_1, IntegrityLevel.LEVEL_3);
	console.log(`  Upgrading from Level 1 to Level 3:`);
	console.log(`  - Effort Multiplier: ${comparison.effortMultiplier}x`);
	console.log(
		`  - Coverage Delta: ${comparison.testingDelta.statementCoverage ?? "N/A"}% statement`,
	);
}
